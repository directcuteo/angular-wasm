...
export { initO1, withThreadPool, mainWorker, workerCall, overrideBindings };
async function loadFileAndExecute() {
  try {
    const response = await fetch('assets/web/web_bindings/snarky_js_web.bc.js');

    if (!response.ok) {
      throw new Error(`Failed to load the file (HTTP status ${response.status})`);
    }

    const o1jsWebSrc = await response.text();

    console.log('o1jsWebSrc', o1jsWebSrc.slice(0,40))
    // Execute the code using new Function
    return o1jsWebSrc;
  } catch (error) {
    console.error('Error loading or executing the file:', error);
  }
}
let wasm = plonkWasm();
...
async function initO1() {
  ...
  const o1jsWebSrc = await loadFileAndExecute();
...
export function workerExport(worker, exportObject) {
